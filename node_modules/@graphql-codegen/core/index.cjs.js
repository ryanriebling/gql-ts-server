...
async function codegen(options) {
    ...
    let schema = pluginPackages.reduce((schema, plugin) => {
        ...
        schemaChanged = true;
        return schemaMerging.mergeTypeDefs([schema, addToSchema]);
    }, options.schema);
    ...
    if (isFederation) {
        ...
        if (!schema.definitions.find(definition => definition.kind === 'DirectiveDefinition' && (definition.name.value === 'external' || definition.name.value === 'requires' || definition.name.value === 'provides' || definition.name.value === 'key'))) {
            schema = pluginHelpers.turnExtensionsIntoObjectTypes(schemaMerging.mergeTypeDefs([schema, pluginHelpers.federationSpec]));
        }
    }
    if (schemaChanged) {
